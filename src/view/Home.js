...
export default function Home() {
  ...
  const [currentLocation, setCurrentLocation] = useState({ latitude: 43.78591, longitude: -79.28986 })//Bonis Ave
  // const [ currentLocation, setCurrentLocation ] = useState({latitude: 43.773350, longitude: -79.336000})//Lambton
  ...
  useEffect(() => {
    (async () => {
      ...
      let { status } = await Location.requestForegroundPermissionsAsync().catch(e => console.log(e));
      if (status !== 'granted') {
      ...
      await Location.getCurrentPositionAsync({}).then(location => setCurrentLocation(location.coords)).catch(e => console.log(e));
  ...
  const onCardPress = (e) => {
    mapRef.current.animateToRegion({ latitude: e.latitude, longitude: e.longitude }, 350)
  }
